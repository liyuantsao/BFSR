...
def main():
    ...
    prior_model_spec = torch.load(opt['prior_model_path'])
    prior_model = models.make(prior_model_spec['prior_model'], load_sd=True).cuda()
    ...
    test_dir = os.path.join(this_dir, '..', 'results', 'SRFlow-LP')
    ...
    for lr_path, hr_path, idx_test in zip(lr_paths, hr_paths, range(len(lr_paths))):
        ...
        with torch.no_grad():
            prior_model.eval()
            lr_up = F.interpolate(lr_t, scale_factor=scale, mode='bilinear', align_corners=False)
            epses_lr = []
            _ = model.get_encode_z(lr_t, lr_up, epses=epses_lr, add_gt_noise=False)
            
            epses = [eps.detach() for eps in epses_lr]
            for i in range(len(epses)):
                mean = torch.mean(epses[i], dim=[1], keepdim=True)
                std = torch.std(epses[i], dim=[1], keepdim=True)
                epses[i] = (epses[i] - mean) / (std + 1e-8)

            epses_learned = prior_model(epses)
            sr_t = model.get_sr(lq=lr_t, epses=epses_learned)

        sr = rgb(torch.clamp(sr_t, 0, 1))
        sr = sr[:h * scale, :w * scale]

        path_out_sr = os.path.join(test_dir, "{:06d}.png".format(idx_test))
        meas = OrderedDict(conf=conf, name=idx_test)
